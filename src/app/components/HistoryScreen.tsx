...
import { Search, ChevronRight, AlertCircle, AlertTriangle, CheckCircle } from "lucide-react";

const diagnoses = [
  {
    id: 1,
    date: "2026-05-03",
    fault: "Fallo en bujías de encendido",
    severity: "moderate",
  ...
  },
  {
    id: 2,
    date: "2026-04-28",
    fault: "Desgaste en pastillas de freno",
    severity: "critical",
    moto: "Kawasaki Ninja 400",
  },
  {
  ...
  },
  {
    id: 4,
    date: "2026-04-10",
    fault: "Cadena de transmisión floja",
    severity: "moderate",
    moto: "Kawasaki Ninja 400",
  },
  {
...
export function HistoryScreen() {
  ...
  const filteredDiagnoses = diagnoses.filter((d) => {
    ...
    if (searchQuery && !d.fault.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-sm text-[#888888] mb-2">{diagnosis.moto}</p>
                      <div className="flex items-center gap-3">
                        <span className={`text-xs px-2 py-1 rounded-full ${config.bg}`} style={{ color: config.color }}>
                          {config.label}
                        </span>
                        <span className="text-xs text-[#888888]">{formatDate(diagnosis.date)}</span>
                      </div>
                    </div>
                  </div>
                </button>
